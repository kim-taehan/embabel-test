...
LLM 은 *각 단계 자체는 잘 수행* 하지만, *어떤 순서로 단계를 밟을지 결정* 하는 일은 잘 못합니다.

#### 한 컷으로 — "ChatGPT 에게 계획 짜기를 시켜보면"

> 같은 질문 *"서울에서 부산까지 5단계 여행 계획 짜줘"* 를 ChatGPT 에 두 번 던지면 **두 번 다 다른 답** 이 옵니다.
> 각 단계 자체는 그럴듯한데, **같은 입력에 같은 결과가 안 나옵니다**.

이게 LLM 플래닝의 본질적 문제 3가지로 이어집니다.

| 문제 | 의미 |
|------|------|
| **비결정적** | 매번 결과가 달라짐 → 운영 시스템에 못 박음 |
| **비쌈** | LLM 호출 한 번 = 돈 + 1~5초 → 단계 결정에 LLM 쓰면 비용 폭발 |
| **디버깅 불가** | 왜 *이번엔* 이 순서로 짰는지 추적할 방법 없음 |

→ Embabel 의 답: **두 책임을 갈라놓자.**

| 책임 | 누가 담당 |
|------|-----------|
| **무엇을 / 어떤 순서로** 할지 결정 | **GOAP 플래너** (코드 — 같은 입력이면 *매번 같은 순서*) |
| **각 단계를 어떻게 수행** 할지 | **LLM** (`@Action` 함수 안) |

#### GOAP 가 뭔데?

**GOAP = Goal-Oriented Action Planning** (목표 지향 행동 계획).

**원조 사례 — 2005년 FPS 게임 *F.E.A.R.*** 의 적 AI 에 처음 적용된 알고리즘입니다.
적 병사에게 *"플레이어를 처치한다"* 라는 **목표 하나만** 주면, **엄폐물 찾기 → 측면으로 이동 → 사격 → 재장전** 같은 행동 순서를 *상황에 따라 자동으로* 짜냅니다. 게이머들이 *"적이 지능적으로 보인다"* 고 평가한 이유.

Embabel 은 같은 알고리즘을 AI 에이전트에 가져왔습니다 — *"보고서 한 편 만들어줘"* 라는 목표만 주면, **계획 수립 → 검색 → 글쓰기 → 조립** 의 순서가 자동으로 짜입니다 (§3, §4 에서 자세히).

### 왜 JVM — 언어 포지셔닝 한 페이지

#### 흔한 오해 — *"AI = Python 아닌가?"*

> ChatGPT 가 뜨고 LangChain 이 뜨면서 *"AI 한다 = Python 쓴다"* 가 굳어진 분위기지만, **현실은 그 반대** 입니다.
>
> **세상에서 가장 많이 쓰는 AI 도구는 Python 으로 만들어지지 않았습니다.**
> - **ChatGPT 웹 / 데스크탑 앱** → TypeScript / React
> - **Claude Code** → TypeScript
> - **Cursor / Continue.dev / OpenCode** → TypeScript
> - **GitHub Copilot 확장** → TypeScript (VS Code 자체가 TS)
> - **OpenAI, Anthropic, Google 의 백엔드 인프라 상당 부분** → Go / Rust / Java
>
> Python 은 *"AI 모델을 만들고 실험하는 영역"* 에서 압도적이지만, *"AI 모델을 사용자에게 전달하는 도구"* 영역에서는 거의 안 쓰입니다.

→ 즉 **"어떤 언어가 AI 에 맞느냐"** 가 아니라, **"어디에 쓸 AI 도구냐"** 가 언어를 결정합니다.

#### 한 컷으로 — "AI 도구는 3종류, 각자 자기 언어가 있다"

| 비유 | 어디서 쓰이나 | 적합한 언어 | 대표 도구 |
|------|-------------|-----------|----------|
| 📱 **사용자 데스크탑 도구** | 사용자 노트북에서 *터미널/IDE 안* 에서 돈다 | **TypeScript, Rust** | Claude Code, Cursor, Copilot |
| 🧪 **연구·실험 노트북** | 데이터 과학자가 Jupyter 에서 모델 시험 | **Python** | LangChain, LangGraph, PyTorch |
| 🏢 **사내 서버에 박는 시스템** | 24시간 운영, 인증·트랜잭션·모니터링 필요 | **JVM (Java/Kotlin)** | **Embabel**, Spring AI |

Embabel 은 **마지막 칸** — *"이미 Java/Kotlin 으로 돌고 있는 회사 서버에 AI 기능 끼워넣기"* 자리에 만들어졌습니다.

#### 케이스 스터디 — Claude Code 는 왜 TypeScript 인가?

Anthropic 이 *"AI 회사인데 왜 자체 도구는 Python 이 아니지?"* 의 답입니다.

| 요구사항 | Python 의 한계 | TypeScript 가 맞는 이유 |
|---------|--------------|----------------------|
| **사용자 노트북 어디에나 설치** | 사용자가 Python 3.11 + venv + pip install 거쳐야 함 → 설치 실패율 ↑ | `npm i -g @anthropic-ai/claude-code` **한 줄**, Node 만 있으면 됨 |
| **즉시 시작 (<200ms)** | Python 인터프리터 + import 시간 (수백 ms) | V8 + 단일 번들 → 빠른 cold start |
| **VS Code / IDE 통합** | VS Code 확장 ≠ Python | **VS Code 자체가 TypeScript** → 같은 생태계 |
| **로컬 파일·Shell·Git 호출이 본업** | Python subprocess 는 가능하지만 어색 | Node 의 `child_process`, `fs` 가 표준 — *시스템 호출이 모국어* |
| **사용자 코드(JS/TS) 자체 분석·편집** | 외부 파서 의존 | TS 컴파일러 API 를 *그대로 import* |

→ **결론** — Anthropic 이 자기 회사 모델을 호출하는 클라이언트 도구를 만들 때조차 **Python 을 안 골랐습니다**. *AI 회사라서 Python* 이 아니라, *어디서 도는가* 가 언어를 결정한다는 가장 강력한 증거.
...
| 카테고리 | 핵심 요구 | 왜 그 언어인가 |
|----------|---------|--------------|
| **사용자 도구** (Claude Code, Cursor) | *0.1초 안에 켜짐 + 어디서나 설치 + 시스템 호출* | TS/Rust — JVM 은 시작이 느리고 JDK 설치 강요해서 **부적합** |
| **연구·실험** (LangChain) | *Jupyter + PyTorch + 빠른 시제품* | Python — 다만 인증·트랜잭션·장애 복구 같은 *운영 표준이 약해* 그대로 사내 서버에 못 올림 |
| **사내 서버 시스템** (Embabel) | *Spring 인증·DB·모니터링과 결합 + 24시간 무중단* | **JVM** — 새 인프라 안 만들고 *기존 회사 서버에 그대로 끼워넣기* 가능 |

#### 한 줄 정리

> **"AI = Python" 은 영역의 한 면만 본 오해.** Python 은 *모델 실험* 의 왕, TypeScript 는 *사용자 도구* 의 왕, JVM 은 *사내 서버 시스템* 의 왕.
> 본 프로젝트는 *사내 Spring 인프라 + 사내망 LLM 게이트웨이* 라는 조건에서 출발했으므로, 처음부터 **JVM 자리에 들어가는 게 맞습니다** — 그 자리에서 가장 성숙한 옵션이 Embabel + Spring AI.
